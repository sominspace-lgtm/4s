  ...
  return (
    <div className="card-interactive" style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '14px', padding: '1.4rem 1.6rem' }}>
      ...
      {subs.map(s => {
        ...
        return (
          <div key={s.id} style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', padding: '0.55rem 0', borderBottom: '1px solid var(--faint)' }}>
            <button
            ...
            >{u === 'paid' ? '✓' : ''}</button>
            <span style={{ fontSize: '0.82rem', color: 'var(--text)', flex: 1, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s.name}</span>
            <span style={{ fontSize: '0.72rem', color: 'var(--muted)', whiteSpace: 'nowrap', flexShrink: 0 }}>{priceLabel}</span>
            <span style={{ fontSize: '0.68rem', color: u === 'fine' || u === 'paid' ? 'var(--muted)' : URGENCY_COLOR[u], whiteSpace: 'nowrap', flexShrink: 0, textAlign: 'right' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginTop: '0.9rem', paddingTop: '0.9rem', borderTop: '1px solid var(--faint)' }}>
        <input value={name} onChange={e => setName(e.target.value)} placeholder="Service name" aria-label="Service name" style={{ ...inputStyle, width: '100%', boxSizing: 'border-box' }} />
        <div style={{ display: 'flex', gap: '0.5rem' }}>
          <input value={cost} onChange={e => setCost(e.target.value)} type="number" step="0.01" placeholder={cadence === 'yearly' ? '$/yr' : '$/mo'} aria-label="Cost" style={{ ...inputStyle, flex: 1, minWidth: 0 }} />
          <input value={date} onChange={e => setDate(e.target.value)} type="date" aria-label="Renewal date" style={{ ...inputStyle, flex: 1.4, minWidth: 0, colorScheme: 'dark' }} />
        </div>
        <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <button onClick={handleAdd} style={{ marginLeft: 'auto', padding: '0.45em 1.1em', borderRadius: '8px', border: '1px solid color-mix(in srgb, var(--gold) 30%, transparent)', background: 'color-mix(in srgb, var(--gold) 8%, transparent)', color: 'var(--gold)', fontFamily: 'var(--font-body)', fontSize: '0.74rem', cursor: 'pointer' }}>Add</button>
        </div>
      </div>
    </div>
  )
