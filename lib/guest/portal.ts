...
export type GuestKind = 'photo' | 'thank_you' | 'guestbook' | 'note' | 'song' | 'from' | 'fridge' | 'wish' | 'spot'
...
export const KIND_FIELDS: Record<GuestKind, { body: boolean; meta: string[] }> = {
  ...
  // A place a guest recommends (2026-09-08). `body` is why they love it,
  // `place` is the name. Lands in a "guests suggested" list the hosts
  // review later — never a pin on its own.
  spot: { body: true, meta: ['place'] },
}
